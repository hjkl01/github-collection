...
Actions Runner Controller (ARC) 是一个 Kubernetes 操作符，用于编排和扩展 GitHub Actions 的自托管 Runner。它支持根据工作流数量自动创建和伸缩基于容器的临时 Runner 规模集，实现弹性扩展。用户可通过 Helm 在 Kubernetes 上部署 ARC，以管理仓库、组织或企业级别的自托管执行环境。