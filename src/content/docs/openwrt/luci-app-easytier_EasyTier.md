...
这是一个适用于 OpenWrt 系统的 LuCI 管理插件，用于配置和管理 EasyTier 网络服务。插件需依赖 kmod-tun 内核模块，且不包含二进制程序（需通过管理界面手动上传）。支持使用 opkg 或 apk 包管理器安装，也可基于 OpenWrt SDK 自行编译，并提供了解决特定日志报错的修复命令。