...
2. **使用方法**：用户可通过克隆仓库，按需导入对应模块（如`tree/bst`或`strings/knuth_morris_pratt`）直接调用算法实现。  
3. **主要特性**：  
   - 模块化设计，按数据结构和算法分类组织代码（如`tree`、`strings`、`heap`等目录）。  
   - 包含常见算法的多种变体（如排序的快速排序、归并排序，树的遍历方式等）。  
   - 部分算法附带示例（如`bst_closest_value`、`path_sum`等），便于理解和应用。  
