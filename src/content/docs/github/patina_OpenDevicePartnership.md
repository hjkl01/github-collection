
---
title: patina
---

### [OpenDevicePartnership patina](https://github.com/OpenDevicePartnership/patina)  ![GitHub Repo stars](https://img.shields.io/github/stars/OpenDevicePartnership/patina?style=social)

Patina 是一个使用 Rust 实现的 UEFI 固件项目，旨在将核心固件组件从 C 迁移到纯 Rust，以提升安全性和稳定性并保留启动性能。项目提供 Patina DXE Core 和 SDK，支持平台集成与组件开发，兼容 aarch64 及 x64 架构。目前处于 Beta 阶段，主要致力于功能稳定化、组件扩展及生态系统整合。