
---
title: MoneyPrinter
---

### [FujiwaraChoki MoneyPrinter](https://github.com/FujiwaraChoki/MoneyPrinter)  ![GitHub Repo stars](https://img.shields.io/github/stars/FujiwaraChoki/MoneyPrinter?style=social)

MoneyPrinter 是一款自动化生成 YouTube Shorts 的工具，通过提供视频主题即可创作内容。项目基于本地 Ollama 模型实现脚本生成与元数据处理，采用 Docker 架构集成数据库生成队列、API 及 Worker，确保任务处理可靠且重启安全。提供交互式安装脚本及详细文档支持。