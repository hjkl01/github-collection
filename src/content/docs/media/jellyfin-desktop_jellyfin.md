...
Jellyfin Desktop 是基于 Qt WebEngine 和 libmpv 构建的 Jellyfin 桌面客户端。支持音频直通、硬件解码及多种格式的直接播放（无需转码）。兼容 Windows、macOS 和 Linux 平台，提供稳定版与开发版构建，并说明各平台配置文件、缓存及日志的存储路径。