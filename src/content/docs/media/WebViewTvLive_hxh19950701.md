
---
title: WebViewTvLive
---

### [hxh19950701 WebViewTvLive](https://github.com/hxh19950701/WebViewTvLive)  ![GitHub Repo stars](https://img.shields.io/github/stars/hxh19950701/WebViewTvLive?style=social)

本项目是一款基于腾讯 X5 WebView 的电视直播应用，通过直接加载官方网页链接实现电视直播观看。主要功能包括自动更新频道列表、支持多直播源及 m3u8 协议，适配手机、平板、电视和车机设备，提供数字键换台、按键反转及自定义设置。应用稳定可靠，但对设备系统版本、硬件配置及 WebView 版本有特定要求。